...
sw_bootstrap_run() {
  ...
  # Host hostname for the cert-backup filename — --network=host shares
  # the net namespace only, not uts, so hostname inside the container
  # would otherwise be the container id. Cheap to pass unconditionally.
  env_args+=(-e "SW_HOST_HOSTNAME=$(hostname -s 2>/dev/null || echo host)")
...
# Extra docker -v flags populated for cert commands; empty otherwise.
# Must be declared before sw_bootstrap_run references it.
SW_CERT_MOUNTS=()

# Repack args for cert commands: resolve host paths to absolutes, map
# them to container paths, set SW_CERT_MOUNTS so sw_bootstrap_run adds
# the right bind mounts. Stores the rewritten arg list in
# SW_CERT_CLI_ARGS. Cert ops never need Consul/Vault discovery.
sw_cert_fast_path() {
  local subcmd="$1"; shift
  SW_HEADLESS=1
  SW_WRITABLE_MANIFEST=0
  SW_CERT_MOUNTS=()
  SW_CERT_CLI_ARGS=("${subcmd}")

  if [ "${subcmd}" = "backup-cert" ]; then
    local out_dir="${1:-$(pwd)}"
    if ! out_dir="$(cd "${out_dir}" 2>/dev/null && pwd)"; then
      sw_err "backup-cert: output dir '${1:-$(pwd)}' does not exist"
      return 2
    fi
    SW_CERT_MOUNTS+=(-v "${out_dir}:/cert-out")
    # Cert sources are read-only for backup.
    [ -d /etc/stackwiz/tls ] && SW_CERT_MOUNTS+=(-v /etc/stackwiz/tls:/etc/stackwiz/tls:ro)
    [ -d /etc/letsencrypt ]  && SW_CERT_MOUNTS+=(-v /etc/letsencrypt:/etc/letsencrypt:ro)
    SW_CERT_CLI_ARGS+=(/cert-out)
    return 0
  fi

  if [ "${subcmd}" = "restore-cert" ]; then
    local force=""
    ...
    local abs_tar tar_dir tar_name
    abs_tar="$(cd "$(dirname "${tarball}")" && pwd)/$(basename "${tarball}")"
    tar_dir="$(dirname "${abs_tar}")"
    tar_name="$(basename "${abs_tar}")"
    SW_CERT_MOUNTS+=(-v "${tar_dir}:/cert-in:ro")
    # Restore writes into these — read-write, and ensure the parents
    # exist on the host so docker doesn't create them as root owned
    # by an unexpected mode.
    [ -d /etc/stackwiz ]     || sudo mkdir -p /etc/stackwiz
    [ -d /etc/letsencrypt ]  || sudo mkdir -p /etc/letsencrypt
    SW_CERT_MOUNTS+=(-v /etc/stackwiz:/etc/stackwiz)
    SW_CERT_MOUNTS+=(-v /etc/letsencrypt:/etc/letsencrypt)
    [ -n "${force}" ] && SW_CERT_CLI_ARGS+=("${force}")
    SW_CERT_CLI_ARGS+=("/cert-in/${tar_name}")
    return 0
  fi
}
...
sw_bootstrap_main() {
  ...
  # Cert commands: minimum host prep (sudo + docker + image) then
  # pass-through to the engine. They don't need Consul/Vault discovery,
  # package setup, or any of the normal install plumbing — the engine
  # itself is the single dispatcher, bootstrap just wires the sandbox.
  case "${1:-}" in
