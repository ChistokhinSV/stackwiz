...
stackwiz_tls_try_cloudflare() {
    ...
    creds="$(mktemp)"
    chmod 600 "$creds"
    echo "dns_cloudflare_api_token = ${CF_DNS_API_TOKEN}" > "$creds"
...
stackwiz_tls_ensure() {
    ...
    if [ "$mode" = "auto" ] || [ "$mode" = "letsencrypt" ]; then
        if stackwiz_tls_try_cloudflare "$host" "$email"; then return 0; fi
        if stackwiz_tls_try_route53    "$host" "$email"; then return 0; fi
        if stackwiz_tls_try_standalone "$host" "$email"; then return 0; fi
